...
class StandingEnv(gym.Env):
    ...
    def __init__(self, xml_path, render_mode=None, max_step=parameters["max_step"]):
        ...
        # 観測空間：位置、速度、角度、角速度
        high = np.array([self.ANGLE_THRESHOLD, np.finfo(np.float32).max, self.MAX_VEL, 
    ...
    # gyro_meas_error: ジャイロスコープの計測誤差 (deg/s)
        ...
        # calculate bike velocity from angular velocity and wheel steering angle
        real_vel = self.DATA.sensordata[self.adr_F_vel]
        return np.array([np.deg2rad(imu[0]), angular_vel, angular_acc, action_steer, action_back, self.target_vel, real_vel], dtype=np.float32)

    # バイクの傾きと位置の変化から報酬を決定
    def _reward(self, obs):
        imu, angular_vel, angular_acc, action_steer, action_back, target_vel, real_vel = obs
        reward = 0.0
        # """ 
        ...
        # reduce reward when velocity and steering angle is differ from target value
        reward -= 0.1 * abs(target_vel - real_vel)
        real_angle = action_steer
        reward -= 0.1 * abs(self.target_angle - real_angle)
        return reward
        # """   
    ...
    def step(self, action):
        ...
        # sometimes, the bike can reach the target velocity by itself without learning.
        self.target_vel = 0
        self.target_angle = 0
