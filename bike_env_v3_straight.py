...
class StandingEnv(gym.Env):
    ...
    def __init__(self, xml_path, render_mode=None):
        ...
        self.f_wheel_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, "tire_holder_yaw")
        ...
        high = np.array([self.ANGLE_THRESHOLD, np.finfo(np.float32).max, 
                         ...
                         np.finfo(np.float32).max, np.finfo(np.float32).max, self.MAX_STEER]) # 角度、角速度、角加速度、前回のアクション、前々回のアクション、オドメトリ
    ...
    def _get_obs(self):
        ...
        if(self.TARGET_xVEL != 0):
            normalized_diff_vel = abs(self.TARGET_xVEL - 0.031 * drive_vel) / abs(self.TARGET_xVEL)  # 速度偏差の正規化: 目標速度とのズレを -1.0 ~ 1.0 の範囲に収める
        elif(self.TARGET_xVEL == 0):
            normalized_diff_vel = abs(0.031 * drive_vel / self.cmd_cfg["max_vel"])

        self.total_theta += self.DT * 0.031 * drive_vel * np.tan(steer_pos) / 0.1605
        delta_X = 0.031 * drive_vel * np.sin(steer_pos) * self.DT * np.cos(self.total_theta)          
        delta_Y = 0.031 * drive_vel * np.cos(steer_pos) * self.DT * np.sin(self.total_theta)                        # 簡易的なオドメトリ計算によるY方向（横ずれ）の積算
        self.total_Xpos += delta_X
        self.total_Ypos += delta_Y
        self.obs = self.total_Xpos

        return np.array([self.filtered_roll, angular_vel, drive_vel, 
                         target_drive_vel, self.total_Ypos, normalized_diff_vel, steer_pos], dtype=np.float32)
    ...
    def _reward(self, obs, action):
        ...
        if(self.TARGET_xVEL != 0):
            ...
            normalized_diff_vel = abs(0.031 * drive_vel / self.cmd_cfg["max_vel"])
        normalized_angle = ((np.deg2rad(45) - abs(imu)) / np.deg2rad(45))                       # 姿勢角の正規化: 45度(制限値)を1.0とし、直立(0度)に近いほど1.0、倒れるほど0.0に近づく
        ...
        r_Ytotal = self.reward_cfg["Ypos_penalty"] * min(0.02, abs(self.total_Ypos)) * 100
    ...
    def step(self, action):
        ...
        if self.env_cfg["real_syncro_noise"] == True:
            ...
            if action_torque > bias:
                ...
                action_torque = 0.0   
            self.control_queue.append(np.array([action_torque]))
            action_torque = self.control_queue[0][0]
        ...
        terminated = bool(abs(obs[0]) > self.ANGLE_THRESHOLD 
                          or (abs(self.total_Ypos) > 0.005 and self.total_Xpos < -0.03)
